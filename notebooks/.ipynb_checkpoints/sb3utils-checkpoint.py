            ...
            for state in current_states:
                ...
                for action_idx, is_safe in enumerate(safe_actions):
                    if is_safe:
                        ...
                        # Note: This assumes you have a method to get next state
                        # print("all attr ",dir(self.env))
                        next_state,x,y,z,w = new_env.step(action_idx)
                        print("symb state",new_env.get_symbolic_state())
                        # print(x)
                        # print(y)
                        # print(z)
                        # print(w)
...
#############################
    def get_shield_action(self, state) -> list:
        ...
        """
        print("Inside get_sheild_actions()")
        self._total_steps += 1
        is_blanked = self.simulator.step()
        # print(state)
        # print("is blank " + str(is_blanked))  # Ensure is_blanked is converted to a string
        # print("last known " + str(self._last_known_state))  # Ensure _last_known_state is converted to a string
        # print("hi")
        # print(type(self.shield))
        # self.shield['bastard']=10
        # print(type(self.shield.keys()))
        # print(self.shield.keys()[0])
        # for key in self.shield:
            # print("key1",type(key))
            # print(key)
            # break
        print(state)
        # print(self.shield[state])
        # print(self.shield[self._last_known_state])  # Ensure shield[state_to_use] is converted to a string
        print("hello")
    ...
    def create_action_mask(self):
        # try:
            ...
            if answer_list == [0.0] * 7:
                sys.exit("No safe actions possible, shield stops.")
            return answer_list
        # except:
