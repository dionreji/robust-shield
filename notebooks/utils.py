...
class MiniGridShieldHandler(ShieldHandler):
    ...
    def __create_shield_dict(self):
        ...
        for stateID in model.states:
            ...
            # print("Old Shield's Actions ",get_allowed_actions_mask([choice_labeling.get_labels_of_choice(model.get_choice_index(stateID, choice[1])) for choice in choices]))
...
class PerceptionLossSimulator:
    ...
    def step(self) -> bool:
        ...
        """
        print("Stats of simulator",self.get_stats())
        print("Moving to next step in simulator")
        self.total_steps+=1
        ...
        if is_blanked:
            self.blanked_count += 1
            print("Agent blanked out at",self.total_steps)
        return is_blanked
